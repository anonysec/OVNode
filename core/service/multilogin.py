...
# Environment file sourced by the client-connect hook for the GLOBAL
# (cross-node) max-login check against the OVManager panel. Contains the node
# API key, so it is readable only by root and the OpenVPN runtime user.
MLOGIN_ENV_FILE = os.path.join(SCRIPTS_DST_DIR, "ovnode-mlogin.env")
...
def _write_mlogin_env() -> None:
    """Write the env file the connect hook sources for the global check.

    The hook authenticates to the panel's GET /mlogin/status/{username}
    endpoint with the node name (X-Node-Name header) and this node's API key
    (key header) — the same credentials the panel stores for this node.
    An empty PANEL_URL disables the global check entirely.
    """
    try:
        from core.config import settings

        panel_url = (settings.panel_url or "").strip().rstrip("/")
        content = (
            "# Generated by OVNode on startup — do not edit (see .env PANEL_URL)\n"
            f"PANEL_URL={panel_url}\n"
            f"NODE_NAME={settings.node_name}\n"
            f"NODE_API_KEY={settings.api_key}\n"
            f"GLOBAL_FAIL_OPEN={'0' if settings.ovnode_global_fail_closed else '1'}\n"
        )
        os.makedirs(SCRIPTS_DST_DIR, exist_ok=True)
        old = None
        if os.path.exists(MLOGIN_ENV_FILE):
            old = Path(MLOGIN_ENV_FILE).read_text(encoding="utf-8")
        if old != content:
            Path(MLOGIN_ENV_FILE).write_text(content, encoding="utf-8")
        # Contains the API key: readable by root + the hook runtime user only.
        user, group = _openvpn_runtime_user_group()
        try:
            shutil.chown(MLOGIN_ENV_FILE, user=user, group=group)
        except Exception as e:
            logger.warning("multilogin: failed to chown %s: %s", MLOGIN_ENV_FILE, e)
        os.chmod(MLOGIN_ENV_FILE, 0o400)
    except Exception as e:
        logger.error("multilogin: failed to write mlogin env: %s", e)
